{function name="categories_top" level=0}
    ...
    <ul class="menu--list menu--level-{$level} columns--{$columnCount}" {if $level === 0}"{/if}> {*style="width: {$menuSizePercentage}%;*}
        {block name="frontend_plugins_advanced_menu_list"}
            {foreach $categories as $category}
                ...
                <li class="menu--list-item item--level-{$level} {if $category.sub}extended-submenu-item{/if}"> {*{if $level === 0} style="width: 100%"{/if}*}
